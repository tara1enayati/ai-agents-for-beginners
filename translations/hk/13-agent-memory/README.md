...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-18T16:03:23+00:00",
...
在本課程中，我們將探討AI代理的記憶是什麼，以及如何管理和利用記憶來提升應用程式的效能。
...
從本質上來說，**AI代理的記憶指的是使其能夠保留和回憶信息的機制**。這些信息可以是對話的具體細節、用戶偏好、過去的行動，甚至是學到的模式。

如果沒有記憶，AI應用程式通常是無狀態的，意味著每次互動都從零開始。這會導致重複且令人沮喪的用戶體驗，因為代理會“忘記”之前的上下文或偏好。
...
這種專門的記憶類型幫助代理建立一致的“個性”或“角色”。它使代理能夠記住關於自身或其預期角色的細節，使互動更加流暢和專注。
...
存儲和管理代理記憶的一種方法是使用專門的工具如Mem0。Mem0作為持久記憶層，允許代理回憶相關互動、存儲用戶偏好和事實上下文，並隨時間從成功和失敗中學習。這裡的理念是將無狀態代理轉變為有狀態代理。
...
Azure AI Search支持如**結構化RAG**的功能，這在從大型數據集（如對話歷史、電子郵件或圖片）中提取和檢索密集結構化信息方面表現出色。與傳統的文本分塊和嵌入方法相比，這提供了“超人級的精確性和召回率”。
